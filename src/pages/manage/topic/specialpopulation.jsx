...
import PopulationSumItem from './sumPie/sumPie'
import PopulationInOutLine from './populationInOutLine/populationInOutLine'
...
class topicManage extends PureComponent {
  ...
  render () {
    const { isRequestPlaceTopicEnd, getdetailedInformationList, counts
      ...
      , grossPopulation
      , floatingList, floatCount, floatNationList, floatPlacelist
      , netFlow, netflows, outFloatingList, outSum, lineIn, lineOut, lineX
    ...
    return (
      <div>
        <Tabs onChange={callback} type="card">
          <TabPane tab="总体情况查询" key="1">
            <SearchForm
              formConfig={this.formConfig}
              doSearch={(values) => { this.getDataList(0, values) }}
            />
            <Row className="population-all-row">
              <Col span={1}><div className="population-sum-row">人口数量</div></Col>
              <Col span={7} style={{ color: '#ff671b' }}>
                {/* <div className="population-sum-row">人口数量</div> */}
                <PopulationSumItem
                  id="baseChart"
                  totalData={'人口总量'}
                  totalLabel={ grossPopulation }
                  chartDataList={ floatingList }
                />
              </Col>
              <Col span={8}>
                <PopulationSumItem
                  id="baseChart2"
                  totalData={'流动人口总量'}
                  totalLabel={floatCount}
                  chartDataList={floatNationList}
                />
              </Col>
              <Col span={7}>
                <PopulationSumItem
            ...
            <Row className="population-all-row">
              <Col span={1}><div className="population-sum-row">人口流动量</div></Col>
              <Col span={7} style={{ color: '#ff671b' }}>
                <PopulationSumItem
                  id="baseChart4"
                  totalData={'净流量'}
                  totalLabel={netflows}
                  chartDataList={netFlow}
                />
              </Col>
              <Col span={8}>
                <PopulationSumItem
                  ...
                  totalData={'流入总量'}
                  totalLabel={outSum}
                  chartDataList={outFloatingList}
                />
              </Col>
              <Col span={7}>
                <PopulationInOutLine
                  xAxisDataList={lineX}
                  personChartDataList={lineIn}
                  carChartDataList={lineOut}
                />
              </Col>
              <Col span={1}>
              </Col>
